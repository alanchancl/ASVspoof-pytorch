...
LOGICAL_DATA_ROOT = r'E:\code\ASVspoof2019\ASVSPoof2019Data\LA'
PHISYCAL_DATA_ROOT = r'E:\code\ASVspoof2019\ASVSPoof2019Data\PA'
...
class ASVDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.cache_fname = 'cache_{}{}_{}_{}.npy'.format(
        ...
        if os.path.exists(self.cache_fname):
            self.data_x, self.data_y, self.data_sysid, self.files_meta = torch.load(
                self.cache_fname)
            ...
            if os.path.exists(self.cache_matlab_fname):
                ...
                print('Dataset loaded from matlab cache ',
                      self.cache_matlab_fname)
                torch.save((self.data_x, self.data_y, self.data_sysid,
                            self.files_meta),
                           self.cache_fname,
                           pickle_protocol=4)
            ...
            print('Dataset saved to cache ', self.cache_fname)
        if sample_size:
            select_idx = np.random.choice(len(self.files_meta),
                                          size=(sample_size, ),
